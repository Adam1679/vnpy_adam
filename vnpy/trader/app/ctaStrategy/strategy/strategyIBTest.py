...
########################################################################
class ibGatewayTestStrategy(CtaTemplate):
    ...
    def __init__(self, ctaEngine, setting):
        ...
        """Constructor"""
        super(ibGatewayTestStrategy, self).__init__(ctaEngine, setting)
        # print '-----------------setting',setting
        # self.pair = self.choose_pair(setting)
        # fileName2 = 'OANDA_connect.json'
        # try:
        #     f2 = file(fileName2)
        # except IOError:
        #     print('读取ib参数配置出错，请检查')
        # # 解析json文件-----------------------------------------------------
        # mysetting2 = json.load(f2)
        # token = str(mysetting2['token'])
        # accountId = str(mysetting2['accountId'])
        # settingName = str(mysetting2['settingName'])
        # self.api = OandaApiV3(self.pair)
        # api.DEBUG = False 默认False
        # self.api.init(settingName, token, accountId)
        # self.getPos()   # 初始化仓位情况

        self.flag = -1
        # self.r.set(self.pair+'buyFlag', self.flag)
