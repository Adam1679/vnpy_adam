...
class CtaTemplate(object):
    ...
    def banzhuan_query_position(self):
        return self.ctaEngine.banzhuan_query_position(self.vtSymbol)
    ...
    def buy_market(self, price, volume, stop=False):
        """买开市价"""
        return self.sendMarketOrder(CTAORDER_BUY, price, volume, stop)
    ...
    def sell_market(self, price, volume, stop=False):
        """卖昨平市价"""
        return self.sendMarketOrder(CTAORDER_SELL, price, volume, stop)

    def sell_today(self, price, volume, stop=False):
        """卖今平"""
        return self.sendOrder(CTAORDER_SELL_TODAY, price, volume, stop)

    def sell_today_market(self, price, volume, stop=False):
        """卖今平市价"""
        return self.sendMarketOrder(CTAORDER_SELL_TODAY, price, volume, stop)
    ...
    def short_market(self, price, volume, stop=False):
        """卖开市价"""
        return self.sendMarketOrder(CTAORDER_SHORT, price, volume, stop)
    ...
    def cover_market(self, price, volume, stop=False):
        """买昨平市价"""
        return self.sendMarketOrder(CTAORDER_COVER, price, volume, stop)

    def cover_today(self, price, volume, stop=False):
        """买今平"""
        return self.sendOrder(CTAORDER_COVER_TODAY, price, volume, stop)

    def cover_today_market(self, price, volume, stop=False):
        """买今平市价"""
        return self.sendMarketOrder(CTAORDER_COVER_TODAY, price, volume, stop)
    ...
    def sendMarketOrder(self, orderType, price, volume, stop=False):
        """发送市价委托"""
        # if self.trading:
        if 1 < 2:
            # 如果stop为True，则意味着发本地停止单
            if stop:
                ...
                # vtOrderIDList = self.ctaEngine.sendOrder(self.vtSymbol, orderType, price, volume, self)
                # print '+'*30,'vtSymbol','+'*30,'\n',self.vtSymbol
                vtOrderIDList = self.ctaEngine.sendMarketOrder(self.vtSymbol, orderType, price, volume, self)
