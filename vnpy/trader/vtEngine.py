...
class MainEngine(object):
    ...
    def dbQueryLimit(self, dbName, collectionName, d, sortKey='', sortDirection=ASCENDING, limit=10):
        if self.dbClient:
            db = self.dbClient[dbName]
            collection = db[collectionName]

            if sortKey:
                cursor = collection.find(d).sort(sortKey, sortDirection).limit(limit)  # 对查询出来的数据进行排序
            else:
                cursor = collection.find(d)
    ...
    def db_update_one(self, dbName, collectionName, d, flt, upsert=False):
        """向MongoDB中更新数据，d是具体数据，flt是过滤条件，upsert代表若无是否要插入"""
        if self.dbClient:
            db = self.dbClient[dbName]
            collection = db[collectionName]
...
class PositionDetail(object):
    ...
    def updatePosition(self, pos):
        ...
        self.long_position = pos.long_position
        self.short_position = pos.short_position
    ...
    def output(self):
        """"""
        # print self.vtSymbol, '-'*30,'持仓细节','-'*30
        # print('long, total:%s, td:%s, yd:%s' %(self.position, self.market_value, self.available))
