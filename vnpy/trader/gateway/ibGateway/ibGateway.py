...
class IbGateway(VtGateway):
    ...
    def __init__(self, eventEngine, gatewayName='IB'):
        ...
        self.r = redis.Redis(host=redisHost, port=int(redisPort), db=8)  # zls添加，用于传持仓信息
    ...
    def redisSub(self):
        ...
        for item in p.listen():
            if item['type'] == 'message':
                ...
                # redis那边推送json类型'[{"vtSymbol":"au1812","dir":"buy"},{"vtSymbol":"USD.CNH.IDEALPRO","dir":"short"}]'
                self.onRedis(data)
    ...
    def onTick(self, tick):
        super(IbGateway, self).onTick(tick)
        bar = VtBarData()
        bar.symbol = tick.symbol
        bar.vtSymbol = tick.vtSymbol
        bar.gatewayName = self.gatewayName

        # 修改了，待命令行检查。然后oandaGateway也要对应这边的修改
        bar.time = tick.time.split(':')[0] + ':' + tick.time.split(':')[1] + ':00.0'
        bar.date = tick.date
        # print(bar.date,bar.time,type(bar.time))
        bar.close = (tick.bidPrice1 + tick.askPrice1) / 2.0
        self.onKLine(bar)

    def onKLine(self, bar):
        pass

    def onPosition(self, pos):
        # pos.symbol, pos.exchange, pos.vtSymbol, pos.direction, pos.position, pos.price, pos.vtPositionName, pos.gatewayName
        # super(IbGateway, self).onPosition(pos) # 不用这个方法了，在这里把仓位信息存在内存里面供使用。
        if pos.symbol == 'CLF9':
            self.r.set('ussymbol', pos.symbol)
            self.r.set('usposition', pos.position)
            self.r.set('uscost', pos.price)
            # usposition = self.r.get('usposition')
            # if not usposition:
            #     self.r.set('usposition', 0)
            # print(pos.symbol, pos.exchange, pos.vtSymbol, pos.direction, pos.position, pos.price, pos.vtPositionName, pos.gatewayName)
    ...
    def subscribe(self, subscribeReq):
        ...
        # contract.secType = str(subscribeReq.productClass)  # 外汇是CASH。zls加上去的可能不用改成这样
        contract.currency = currencyMap.get(subscribeReq.currency, '')
        # contract.currency = str(subscribeReq.currency)  # zls加上去的可能不用改成这样
    ...
    def sendOrder(self, orderReq):
        ...
        msg1 = 'localSymbol:', contract.localSymbol, ' exchange:', contract.exchange, ' secType:', contract.secType, ' currency:', contract.currency, ' expiry:', contract.expiry, ' strike', contract.strike, ' right:', contract.right, ' lastTradeDateOrContractMonth:', contract.lastTradeDateOrContractMonth, ' multiplier:', contract.multiplier
        print(msg1)
        ...
        msg2 = 'orderId:', order.orderId, ' clientId:', order.clientId, ' action:', order.action, ' lmtPrice:', order.lmtPrice, ' totalQuantity:', order.totalQuantity, ' orderType:', order.orderType
        print(msg2)
...
class IbWrapper(IbApi):
    ...
    def __init__(self, gateway):
        """Constructor"""
        # print('--------ibGateway.py 初始化IbWrapper line 364---------')
    ...
    def error(self, id_, errorCode, errorString):
        ...
        print('------------IbGateway.py error line414 ' + str(err.errorMsg))
    ...
    def tickPrice(self, tickerId, field, price, canAutoExecute):
        ...
        if field in tickFieldMap:
            ...
            if float(tick.bidPrice1) == 0 or float(tick.askPrice1) == 0:
                return  # 这个判断不能放在上面去，会的不到tick

            tick.lastPrice = (tick.bidPrice1 + tick.askPrice1) / 2.0
            dt = datetime.now()
