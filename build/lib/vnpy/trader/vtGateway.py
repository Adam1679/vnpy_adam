...
class VtGateway(object):
    ...
    def onKLine(self, bar):
        """市场行情推送"""
        # 通用事件
        event1 = Event(type_=EVENT_KLINE)
        event1.dict_['data'] = bar
        self.eventEngine.put(event1)

        # 特定合约代码的事件
        event2 = Event(type_=EVENT_KLINE + bar.vtSymbol)
    ...
    def onContract(self, contract):
        """合约基础信息推送"""
        # 通用事件
        # print(1111111111111111111111111111,'vtGateway.py_line111')
        event1 = Event(type_=EVENT_CONTRACT)
        event1.dict_['data'] = contract
        self.eventEngine.put(event1)

    def onSyncOrders(self, orders):
        """定时订单查询事件"""
        event1 = Event(type_=EVENT_SYNC_ORDERS)
        event1.dict_['data'] = orders
        self.eventEngine.put(event1)

    def onRedis(self, data):
        """Redis订阅信息推送"""
        event1 = Event(type_=EVENT_REDIS)
