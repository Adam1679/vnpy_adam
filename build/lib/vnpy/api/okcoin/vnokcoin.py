...
OKEX_CONTRACT = 'wss://real.okex.com:10440/websocket/okexapi'
...
class OkCoinApi(object):
    ...
    def readData(self, evt):
        ...
        #
        # # 将原始数据解压成字符串
        # inflated = decompress.decompr ess(evt) + decompress.flush()
    ...
    def login(self, apiKey, secretKey):
        # 生成请求
        d = {}
        d['event'] = 'login'
        # d['binary'] = '0'
    ...
    def futureTrade(self, symbol, type_, price, amount, expiry='quarter', order='0', leverage='10'):
        """期货委托"""
        params = {}
        params['symbol'] = str(symbol+self.currency)
        params['type'] = str(type_)
        ...
        params['match_price'] = str(order)  # 是否为对手价： 0:不是 1:是 当取值为1时,price无效
        params['lever_rate'] = str(leverage)  # 杠杆倍数 value:10\20 默认10

        # print('futureTrade: {0}'.format(params))
        
        channel = 'ok_future%s_trade' % self.currency
        
        self.sendTradingRequest(channel, params)
        
    def futureCancelOrder(self, symbol, orderid, expiry='quarter'):
        """期货撤单"""
        params = {}
        params['symbol'] = str(symbol+self.currency)
        params['order_id'] = str(orderid)
