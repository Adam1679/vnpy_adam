...
class EventEngine2(object):
    ...
    def __init__(self):
        ...
        self.__timer_2 = Thread(target=self.__runTimer_2)
        self.__timerSleep_2 = 30 #60*30
    ...
    def __runTimer_2(self):
        """运行在计时器线程中的循环函数"""
        while self.__timerActive:
            # 创建计时器事件
            event = Event(type_=EVENT_TIMER_30_MINUTES)

            # 向队列中存入计时器事件
            self.put(event)

            # 等待
            sleep(self.__timerSleep_2)
