...
class DoubleMAStrategy(CtaTemplate):
    ...
    def onBar(self, bar):
        ...
        if self.ctaEngine.engineType == ENGINETYPE_TRADING:
            try:
                postion = self.ctaEngine.query_position()[0]
                if postion['long'] == 0 and postion['short'] == 0:
                    self.pos = 0
                elif postion['short'] > 0:
                    self.pos = -postion['short']
                elif postion['long'] > 0:
                    self.pos = postion['long']
            except Exception as e:
                self.writeCtaLog(u'获取持仓信息失败！{0}'.format(e))
        ...
        if self.pos == 0:
            ...
            if self.ctaEngine.engineType == ENGINETYPE_TRADING:
                volume = int(account_right * bar.close) / 100
                if crossOver:
                    self.buy(bar.close, volume)
                    self.pos = volume  # 防止仓位请求失败而设置的假设仓位
                elif crossBelow:
                    self.short(bar.close, volume)
                    self.pos = -volume  # 防止仓位请求失败而设置的假设仓位
            else:
                volume = int(self.initCapital / 100)
                if crossOver:
                    # if not self.stop:
                        self.buy(bar.close, volume)
                    # self.stop = False
                    ...
                    # if not self.stop:
                        self.short(bar.close, volume*2)
                    # self.stop = False

        # 持有多头仓位
        elif self.pos > 0:
            old_capital = self.initCapital
            new_capital = self.trade_result(bar)
            ...
            if crossBelow:
                ...
                if self.ctaEngine.engineType == ENGINETYPE_TRADING:
                    volume = int(account_right * bar.close) / 100
                    orderID = self.short(bar.close, volume)
                    self.pos = -volume  # 防止仓位请求失败而设置的假设仓位
                else:
                    self.initCapital = self.trade_result(bar)
                    volume = int(self.initCapital / self.contract_size)
                    orderID = self.short(bar.close, volume*2)
        ...
        # 持有空头仓位
            ...
            if crossOver:
                ...
                if self.ctaEngine.engineType == ENGINETYPE_TRADING:
                    volume = int(account_right * bar.close) / 100
                    orderID = self.buy(bar.close, volume)
                    self.pos = volume  # 防止仓位请求失败而设置的假设仓位
                else:
                    self.initCapital = self.trade_result(bar)
                    volume = int(self.initCapital / self.contract_size)
