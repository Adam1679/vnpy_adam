...
class DoubleMAMultStrategy(CtaTemplate):
    ...
    def onBar(self, bar):
        ...
        # 持有多头仓位
            ...
            if self.last_entry_price > 0 and (bar.close - self.last_entry_price)/self.last_entry_price > self.stop_profit:
                self.stop = 4
                sell_order = True
                # print("*"*20)
            elif crossBelow:
        ...
        # 持有空头仓位
            ...
            if self.last_entry_price > 0 and (self.last_entry_price - bar.close) / self.last_entry_price > self.stop_profit:
                self.stop = 4
                cover_order = True
                # print("*" * 20)
            elif crossOver:
