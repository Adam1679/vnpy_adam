...
class BacktestingEngine(object):
    ...
    def newBar(self, bar):
        ...
        #TODO: 这里改成了当前bar下单当前bar成交，可能会使用到一部分的未来数据。
        self.strategy.onBar(bar)  # 推送K线到策略中
        # self.crossOrder()      # 撮合吃单
        self.crossLimitOrder(compound=self.compound)    # 先撮合限价单
    ...
    #----------------------------------------------------------------------
    def crossLimitOrder(self, compound=False):
        """基于最新数据撮合限价单"""
        # compound = True 则意味着使用的复利交易，每一次开仓都是全仓
        # 先确定会撮合成交的价格
        if self.mode == self.BAR_MODE:
            ...
            if buyCross or sellCross:
                ...
                if buyCross:
                    ...
                    if self.strategy.pos == 0:  # 做多
                        ...
                        self.strategy.initCapital -= trade.price * order.totalVolume
                    else:  # 平空
    ...
    def calculateBacktestingResult(self):
        ...
        # 计算是否存在未平的多空单，若有则强平
        vol = self.strategy.pos
        if vol > 0: # 有多单未被处理
            self.sendOrder(self.strategy.vtSymbol, CTAORDER_SELL, self.lastPrice, vol, self.strategy)
            self.output(u'有%d多单未平，用最后收盘价强平'%vol)

        if vol < 0: # 有空单未被处理

            self.sendOrder(self.strategy.vtSymbol, CTAORDER_BUY, self.lastPrice, -vol, self.strategy)
            self.output(u'有%d空单未平,用最后收盘价强平' % (-vol))

        self.crossLimitOrder(compound=False)
