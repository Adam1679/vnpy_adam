...
class AtrRsiStrategy(CtaTemplate):
    ...
    def onBar(self, bar):
        ...
        slippage = self.ctaEngine.slippage
        ...
        if self.pos == 0:
            ...
            if slow_up:
                self.cancelAll()
                volume = round(self.initCapital / (bar.close * (1 + slippage)), 4)
                self.buy(bar.close * (1 + slippage), volume)
                # print("slappage: {0}，{1}".format(bar.close, bar.close * (1 + slippage)))
            elif slow_down:
                self.cancelAll()
                volume = round(self.initCapital / (bar.close * (1 - slippage)), 4)
                self.short(bar.close * (1 - slippage), volume)
    ...
    #     from ctaBacktesting001 import TradingResult
    ...
    def onTrade(self, trade):
        print('%s %s %s, price:%s, volume:%.4f, capital :%.2f' %(trade.dt,trade.offset,trade.direction, trade.price, trade.volume, trade.price * trade.volume))
