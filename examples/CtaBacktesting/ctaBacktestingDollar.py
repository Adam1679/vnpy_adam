...
class BacktestingEngine(object):
    ...
    def runBacktesting(self, position=1, task_text='1/1'):
        ...
        # p.update((float(index) / count * 100))
        # p.finish()
    ...
    def crossOrder(self):
        ...
        for orderID, order in self.workingLimitOrderDict.items():
            # if order_count % 2 != 0 and order_index == order_count:
            #     continue
            ...
            if buyCross or sellCross:
                ...
                # print('pose: {0}'.format(self.strategy.pos))
                if buyCross:
    ...
    def crossLimitOrder(self):
        ...
        for orderID, order in self.workingLimitOrderDict.items():
            ...
            # print(u'{0},{1},{2}'.format(order.price, order.totalVolume, order.direction))
            # 判断是否会成交
            buyCross = (order.direction == DIRECTION_LONG and
    ...
    def calculateBacktestingResult(self):
                ...
                # 当前多头交易为平空
                    ...
                    while True:
                        ...
                        result = TradingResult(entryTrade.price, entryTrade.dt,
                                               ...
                                               -closedVolume, self.rate, self.slippage, 100)
        ...
        for result in resultList:
            # print(init_capital)
            old_init_capital = init_capital
            init_capital = init_capital / result.entryPrice * result.exitPrice + result.pnl * result.exitPrice
            capital += init_capital - old_init_capital
            maxCapital = max(capital, maxCapital)
            maxNetCapital = max(init_capital, maxNetCapital)
            drawdown = init_capital - maxNetCapital
            ...
            if maxNetCapital == 0:
                drawdownRateList.append(0)
                print('maxNetCapital is zero!')
            else:
            ...
            totalCommission += result.commission_dollar
            totalSlippage += result.slippage_dollar

            if result.pnl >= 0:
                winningResult += 1
                totalWinning += init_capital - old_init_capital
            else:
                losingResult += 1
                totalLosing += init_capital - old_init_capital
    ...
    def show_KLine(self, filename='test'):
        ...
        # 载入初始化需要用的数据
        start_date_str = '20171130'
        # end_date_str = '20171230'
        end_date_str = '20180426'
        ...
        fast_ma_length = self.strategy.fastMaLength
        slow_ma_length = self.strategy.slowMaLength
        ma_1 = talib.MA(np.array(kurse_c), fast_ma_length)
        ma_2 = talib.MA(np.array(kurse_c), slow_ma_length)
        ...
        ma1 = go.Scatter(x=kurse_dates, y=ma_1, marker={'color': 'orange'}, name="MA %s" % fast_ma_length)
        ma2 = go.Scatter(x=kurse_dates, y=ma_2, marker={'color': 'blue'}, name="MA %s" % slow_ma_length)
        data = [hour, ma1, ma2]
        layout = go.Layout(
            yaxis=dict(
                fixedrange=False
        ...
        # py.iplot(data, filename='simple_candlestick1111')
        of.offline.plot(fig)
        #
    ...
    def showBacktestingResult(self):
        ...
        trace0 = go.Scatter(x=d['timeList'], y=d['capitalList'], text=d['trade_detail'], marker={'color': 'red'},
                            name="策略收益")
    ...
    # ----------------------------------------------------------------------
    def runParallelOptimization(self, strategyClass, optimizationSetting, condiction=None):
        ...
        if condiction is not None:
            for setting in settingList:
                if not condiction(setting):
                    setting_list2.append(setting)
        else:
            setting_list2 = settingList

        index = 0
        try:
            for setting in setting_list2:
                index += 1
                l.append(pool.apply_async(optimize, (strategyClass, setting,
                                                     targetName, self.mode,
                                                     self.startDate, self.initDays, self.endDate,
                                                     self.slippage, self.rate, self.size,
                                                     self.dbName, self.symbol, (index - 1) % cpu_count + 1,
                                                     '{0}/{1}'.format(index, len(setting_list2)))))
            pool.close()
            pool.join()
        ...
        writer.writerow(['基准收益：%.2f%%'.encode('gbk') % resultList[0][4]])
        print(u'基准收益：%.2f%%' % resultList[0][4])
        first = True
        for result in resultList:
            json_obj = ast.literal_eval(result[0])
            if first:
                first = False
                setting = [key for key, value in json_obj.items()]
                line_csv = setting + ['优化值'.encode('gbk'), '策略收益(%)'.encode('gbk'), '最大回撤率(%)'.encode('gbk'), 'times']
                line_print = setting + ['优化值', '策略收益(%)', '最大回撤率(%)', 'times']
                writer.writerow(line_csv)
                print(str(line_print).decode('string_escape'))

            setting = [value for key, value in json_obj.items()]
            line = setting + [round(result[1], 2), round(result[2], 2), round(result[3], 2), int(result[5])]
...
class TradingResult(object):
    ...
    def __init__(self, entryPrice, entryDt, exitPrice,
        ...
        self.volume = volume  # 交易数量（+/-代表方向）,-平空，+为平多

        self.turnover = size * abs(volume)/self.entryPrice + size * abs(volume) / self.exitPrice  # 成交金额 BTC
        # self.turnover_dollar = size * abs(volume) *2
        self.commission = self.turnover * rate  # 手续费成本 BTC

        self.commission_dollar = size * abs(volume) * rate * 2

        # self.slippage = slippage*2*size*abs(volume)                         # 滑点成本
        self.slippage = slippage * self.turnover
        self.slippage_dollar = size * abs(volume) * slippage * 2
        self.pnl = size * volume/self.entryPrice - size * volume / self.exitPrice \
                   - self.commission - self.slippage
        # self.pnl_dollar = size * volume -
...
def optimize(strategyClass, setting, targetName,
    ...
    try:
        ...
        targetValue5 = d['totalResult']
    except KeyError:
        targetValue = 0

    setting.pop('contract_size', None)
    if 'stop_profit' in setting:
        setting['stop_profit'] = round(setting['stop_profit'], 3)
    return str(setting), targetValue, targetValue2, targetValue3, targetValue4, targetValue5
