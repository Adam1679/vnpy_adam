...
class MACDStrategy(CtaTemplate):
    ...
    def onBar(self, bar):
        ...
        macd, macdsignal, macdhist = talib.MACD(self.closeArray, self.fast_period, self.slow_period, self.signal_period)
        ...
        crossOver = macd[-1] > macdsignal[-1] and macd[-2] <= macdsignal[-2]  # 上穿
        crossBelow = macd[-1] < macdsignal[-1] and macd[-2] >= macdsignal[-2]   # 下穿
        ...
        if self.ctaEngine.engineType == ENGINETYPE_TRADING:
            ...
            for order in incomplete_orders:
                if increasing and order['order_type'] == 3:  # 平空
                    self.short(bar.close, order['order_amount'] - order['deal_amount'])
                elif decreasing and order['order_type'] == 4:  # 平多
                    self.sell(bar.close, order['order_amount'] - order['deal_amount'])
